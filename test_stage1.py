...
def parse_args():
    ...
    parser.add_argument("--test_batch_size", 
        type=int, default=16, help="Batch size (per device) for the testing dataloader."
    )
    ...
    parser.add_argument('--prepare_data_for_t2', type=bool, default=False,help='prepare train data for t2')

    parser.add_argument("--uncond_fraction", type=float, default=0.2, help="Fraction of unconditioned training samples")
    parser.add_argument("--vae_type", type=str, choices=["enhance", "origin"], default='origin',
...
class VTO(pl.LightningModule):
    def __init__(self,                 pretrained_model_name_or_path:str=None,
        ...
        Initialize the class with the specified parameters.
            Parameters:
                ...
                - adam_beta1 (float): Beta1 hyperparameter for the Adam optimizer.
                - adam_beta2 (float): Beta2 hyperparameter for the Adam optimizer.
                - adam_weight_decay (float): Weight decay for the Adam optimizer.
                - adam_epsilon (float): Epsilon value for numerical stability in Adam optimizer.
                - warm_up (float): Number of warm-up steps.
                - uncond_fraction (float): Fraction of unconditional samples.
                - output_dir (str): Directory to save output.
                - guidance_scale (float): Scale factor for guidance loss.
    ...
    def configure_model(self) -> None:
        ...
        #Human UNet
        if self.height == 512:
            self.human_unet = Human_Unet.from_pretrained(self.pretrained_model_name_or_path, subfolder=f"stage1/{self.dataset}/human_unet")
            #Garment UNet
            self.garment_unet=Garment_Unet.from_pretrained(self.pretrained_model_name_or_path, subfolder=f"stage1/{self.dataset}/garment_unet")
            #Garment Encoder
            self.garment_encoder=Garment_Encoder.from_pretrained(self.pretrained_model_name_or_path, subfolder=f"stage1/{self.dataset}/garment_encoder") 
            #Pose Encoder
            self.pose_encoder=PoseEncoder.from_pretrained(self.pretrained_model_name_or_path, subfolder=f"stage1/{self.dataset}/pose_encoder") 
        else:
            self.human_unet = Human_Unet.from_pretrained(self.pretrained_model_name_or_path, subfolder=f"stage1/{self.dataset}-1024/human_unet")
            #Garment UNet
            self.garment_unet=Garment_Unet.from_pretrained(self.pretrained_model_name_or_path, subfolder=f"stage1/{self.dataset}-1024/garment_unet")
            #Garment Encoder
            self.garment_encoder=Garment_Encoder.from_pretrained(self.pretrained_model_name_or_path, subfolder=f"stage1/{self.dataset}-1024/garment_encoder") 
            #Pose Encoder
            self.pose_encoder=PoseEncoder.from_pretrained(self.pretrained_model_name_or_path, subfolder=f"stage1/{self.dataset}-1024/pose_encoder") 
        ...
        self.pose_encoder.eval().requires_grad_(False)
...
if __name__=='__main__':
    ...
    logger=WandbLogger(save_dir=args.output_dir,project='free-tryon-stage-1-512',name='train')
    trainer=pl.Trainer(
        ...
        precision=args.mixed_precision,num_sanity_val_steps=2,gradient_clip_val=1,
