...
class VGGLoss(nn.Module):
    ...
    def forward(self, x, y,num=None):
        x = nn.functional.interpolate(x,256,mode='bilinear',antialias=True)
        y = nn.functional.interpolate(y,256,mode='bilinear',antialias=True)
        x = (x + 1) / 2  # [-1, 1] -> [0, 1]
        y = (y + 1) / 2
