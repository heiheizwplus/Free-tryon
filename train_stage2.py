...
class VTO(pl.LightningModule):
    def __init__(self,                 pretrained_model_name_or_path:str=None,
        ...
        """
            ...
            Parameters:
                ...
                dataset (str, optional): Name of the dataset. Default is 'vitonhd'.
                ...
                vae_type (str, optional): Type of VAE to use. Default is 'enhance'.
    ...
    def configure_model(self) -> None:
        ...
        #unet ：Noise-laten + human laten
        human_unet_new_channels=4+4
        with torch.no_grad():
            # Replace the first conv layer of the unet with a new one with the correct number of input channels
            conv_new = torch.nn.Conv2d(
                in_channels=human_unet_new_channels,
                out_channels=self.human_unet.conv_in.out_channels,
                kernel_size=3,
                padding=1,
            )
            ...
            conv_new.weight.data[:, :4] = self.human_unet.conv_in.weight.data[:,:4]
            conv_new.weight.data[:, 4:] = self.human_unet.conv_in.weight.data[:,5:]  
            conv_new.bias.data = self.human_unet.conv_in.bias.data

            self.human_unet.conv_in = conv_new  # replace conv layer in unet
            self.human_unet.config['in_channels'] = human_unet_new_channels  # update config

        #unet ：Noise-laten + garment laten
        ...
        self.garment_encoder=Garment_Encoder.from_config('./train_config/garment_encoder/config.json')
    ...
    def training_step(self,batch,batch_idx) :
        ...
        tryon_image_latents = self.img_to_laten(batch['tryon_image'])*self.vae.config.scaling_factor  #stage 1 generate tryon images
        ...
        unet_input=torch.cat([noisy_latents,tryon_image_latents],dim=1)
        model_pred=self(unet_input,timesteps,encoder_hidden_states,self_attn_states,None)
    ...
    def configure_optimizers(self):
        params =[i  for i in (list(self.garment_unet.parameters()) +list(self.garment_encoder.parameters())
