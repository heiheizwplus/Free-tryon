...
class StableDiffusionTryOnPipeline(DiffusionPipeline):
    ...
    """
        ...
        human_latent=self.vae.encode(human_image).latent_dist.sample(generator=generator)*self.vae.config.scaling_factor
        human_latents=torch.cat([human_latent]*2)
        ...
        with self.progress_bar(total=num_inference_steps) as progress_bar:
            for i, t in enumerate(timesteps):
                ...
                noise_pred = self.human_unet(latent_model_input, t, encoder_hidden_states=prompt_embeds,self_attn_states=self_attn_states,).sample
