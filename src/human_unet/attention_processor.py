...
from diffusers.utils.torch_utils import logging, maybe_allow_in_graph
...
class Attention(nn.Module):
    ...
    def forward(self, hidden_states, encoder_hidden_states=None, attention_mask=None,self_attn_state=None, **cross_attention_kwargs):
...
class AttnProcessor:
    ...
    def __call__(
        ...
        if input_ndim == 4:
            hidden_states = hidden_states.transpose(-1, -2).contiguous().reshape(batch_size, channel, height, width)
...
class AttnAddedKVProcessor:
    ...
    def __call__(self, attn: Attention, hidden_states, encoder_hidden_states=None, attention_mask=None):
        residual = hidden_states
        hidden_states = hidden_states.view(hidden_states.shape[0], hidden_states.shape[1], -1).transpose(1, 2).contiguous()
        batch_size, sequence_length, _ = hidden_states.shape
        ...
        hidden_states = hidden_states.transpose(-1, -2).contiguous().reshape(residual.shape)
        hidden_states = hidden_states + residual
...
class AttnAddedKVProcessor2_0:
    ...
    def __call__(self, attn: Attention, hidden_states, encoder_hidden_states=None, attention_mask=None):
        ...
        hidden_states = hidden_states.transpose(1, 2).contiguous().reshape(batch_size, -1, residual.shape[1])
...
class AttnProcessor2_0:
    ...
    def __call__(
        ...
        if self_attn_state is not None:
            # print('using xformer')
            encoder_hidden_states=torch.cat((encoder_hidden_states,self_attn_state),dim=1)   #拼接 Garment Unet 提取服装特征和Human UNet的特征
        else:
            # print('self_attn_state is None')
            pass
        ...
        hidden_states = hidden_states.transpose(1, 2).contiguous().reshape(batch_size, -1, attn.heads * head_dim)
        hidden_states = hidden_states.to(query.dtype)
