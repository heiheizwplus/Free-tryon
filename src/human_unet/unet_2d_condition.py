...
from src.human_unet.attention_processor import AttnProcessor
from diffusers.models.embeddings import GaussianFourierProjection, TimestepEmbedding, Timesteps
from diffusers.models.modeling_utils import ModelMixin
from src.human_unet import (
...
class UNet2DConditionModel(ModelMixin, ConfigMixin, UNet2DConditionLoadersMixin):
    ...
    def forward(
        ...
        self_attn_states:Optional[List[List[torch.FloatTensor]]]=None,    # Garment UNet提取的服装特征
        condition_latens:Optional[torch.FloatTensor]=None,  #   pose encoder 提取的姿态特征 tryon model T1 need
        ...
        if condition_latens is not None:  
            sample+=condition_latens
        ...
        for downsample_block in self.down_blocks:
            if hasattr(downsample_block, "has_cross_attention") and downsample_block.has_cross_attention:
                sample, res_samples = downsample_block(
                    ...
                    self_attn_state=self_attn_states[temp_self_attention_index] if self_attn_states is not None else None  #输入Garment UNet提取的服装特征
                )
        ...
        if self.mid_block is not None:
            sample = self.mid_block(
                ...
                attention_mask=attention_mask,
                cross_attention_kwargs=cross_attention_kwargs,
                self_attn_state=self_attn_states[temp_self_attention_index] if self_attn_states is not None else None  #输入Garment UNet提取的服装特
            )
            temp_self_attention_index+=1
        ...
        for i, upsample_block in enumerate(self.up_blocks):
            ...
            if hasattr(upsample_block, "has_cross_attention") and upsample_block.has_cross_attention:
                ...
                temp_self_attention_index+=1  #输入Garment UNet提取的服装特
            else:
                sample = upsample_block(
